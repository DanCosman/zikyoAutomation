...
public class GenericSteps extends TestBase {
    ...
    public void I_click_on_button2_with_text(String text) {
        Button button = new Button().setText("Login", SearchType.DEEP_CHILD_NODE_OR_SELF).setVisibility(true);
        button.assertClick();
    }
...
//        Form parentForm = new Form(visibleContainer).setTitle("Drivers");
    ...
    public void I_click_on_label(String text) throws Throwable {
        WebLocator label = new WebLocator().setText(text).setTag("label");
        label.assertClick();
